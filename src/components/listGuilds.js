            ...
            <div className="flex justify-between hover:bg-gray-600 md:w-128 sm:w-128 p-1" key={i}>
              ...
              <div className="mt-4">
                <button
                  ...
                  onClick={() =>
                    (window.location.href = text === 'Manage' ? `https://hunter-bot-backend.herokuapp.com/server/${guild.id}` : `http://discord.com/oauth2/authorize?client_id=905608828970729482&scope=bot%20applications.commands&guild_id=${guild.id}&response_type=code&redirect_uri=http%3A%2F%2Fdiscord-bot-backend-app.herokuapp.com%2Fapi%2Fauth%2Fdiscord%2Fredirect`)
                  }
                >
                  {text}
                </button>
